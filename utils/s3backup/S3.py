...
class AWSAuthConnection:
    ...
    def list_bucket(self, bucket, options={}, headers={}):
        return ListBucketResponse(self.make_request('GET', bucket, '', options, headers))
    ...
    def get(self, bucket, key, headers={}):
        return GetResponse(
                self.make_request('GET', bucket, key, {}, headers))

    def delete(self, bucket, key, headers={}):
        return Response(
                self.make_request('DELETE', bucket, key, {}, headers))

    def get_bucket_logging(self, bucket, headers={}):
        return GetResponse(self.make_request('GET', bucket, '', { 'logging': None }, headers))

    def put_bucket_logging(self, bucket, logging_xml_doc, headers={}):
        return Response(self.make_request('PUT', bucket, '', { 'logging': None }, headers, logging_xml_doc))
    ...
    def get_acl(self, bucket, key, headers={}):
        return GetResponse(
                self.make_request('GET', bucket, key, { 'acl': None }, headers))
    ...
    def make_request(self, method, bucket='', key='', query_args={}, headers={}, data='', metadata={}):
        ...
        # add auth header
        self.add_aws_auth_header(final_headers, method, bucket, key, query_args)

        if (self.is_secure):
            connection = httplib.HTTPSConnection("%s:%d" % (server, self.port))
        else:
            connection = httplib.HTTPConnection("%s:%d" % (server, self.port))
