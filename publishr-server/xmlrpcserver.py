...
class XmlRpcServer:
    ...
    def handle(self,webapp):
        request = StringIO(webapp.request.body)
        request.seek(0)
        response = StringIO()
        try:
            ...
            response.seek(0)
        rstr = response.read()
        webapp.response.headers['Content-type'] = 'text/xml'
        webapp.response.headers['Content-length'] = "%d"%len(rstr)
        webapp.response.out.write(rstr)
        return True
